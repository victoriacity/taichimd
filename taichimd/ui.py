...
class GUI:
    ...
    def __init__(self, system, shader):
        self.system = system
        self.gui = ti.GUI("MD", res=WINDOW_SIZE)
        self.shader = shader(self.gui)
    ...
    def show(self, savefile=None, draw_bonds=False):
        nbonds = self.system.nbonds if draw_bonds else 0
        self.shader.render(self.system.position.to_numpy() \
...
class CanvasShader(Shader):
    ...
    def render(self, positions, nbonds=0, bond_table=None):
        ...
        for n in range(nbonds):
            i = bond_table[n][0]
            j = bond_table[n][1]
            self.gui.line(xy[i], xy[j], color=rgb2hex(self.bond), radius=self.radius / 3)
